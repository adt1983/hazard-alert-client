...
public class Database {
	...
	public static Hazard cursorToHazard(Cursor c) {
		...
		try {
			...
			//h.alert = Alert.newBuilder().mergeFrom(c.getBlob(c.getColumnIndexOrThrow(HazardTable.COLUMN_ALERT))).build();
			//h.expires = new Date(c.getLong(c.getColumnIndexOrThrow(HazardTable.COLUMN_EXPIRES)));
			//h.setEffective(new Date(c.getLong(c.getColumnIndexOrThrow(HazardTable.COLUMN_EFFECTIVE))));
			//h.id = c.getString(c.getColumnIndexOrThrow(HazardTable.COLUMN_ALERT_ID));
			/*
			h.bbNE = new Point(	c.getDouble(c.getColumnIndexOrThrow(HazardTable.COLUMN_BB_NE_LAT)),
								c.getDouble(c.getColumnIndexOrThrow(HazardTable.COLUMN_BB_NE_LNG)));
			h.bbSW = new Point(	c.getDouble(c.getColumnIndexOrThrow(HazardTable.COLUMN_BB_SW_LAT)),
								c.getDouble(c.getColumnIndexOrThrow(HazardTable.COLUMN_BB_SW_LNG)));
								*/
	...
	public List<Hazard> list(AlertFilter filter) {
		...
		String sql = "SELECT * FROM hazard WHERE 1 = 1";
		if (null != filter.getInclude()) {
			Bounds b = filter.getInclude();
			sql = sql.concat(" AND NOT (bb_ne_lat < ?) AND NOT (bb_ne_lng < ?) AND NOT (bb_sw_lat > ?) AND NOT (bb_sw_lng > ?)");
		...
		if (null != filter.getLimit()) {
			...
			strings.add(filter.getLimit().toString());
		}
		String selectionArgs[] = new String[strings.size()];
		for (int i = 0; i < strings.size(); i++) {
			selectionArgs[i] = strings.get(i);
		}
		List<Hazard> results = cursorToList(db.rawQuery(sql, selectionArgs));
		if (null != filter.getInclude()) {
