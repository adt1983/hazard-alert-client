...
public class Hazard {
	...
	public Hazard(Alert alert, int infoIndex) {
		assert infoIndex >= 0;
		assert infoIndex < alert.getInfoCount();
	...
	private void initFromAlert() {
		...
		assert info.hasLanguage();
		assert info.hasEffective();
		assert info.hasExpires();
		assert info.hasCertainty();
		assert info.hasUrgency();
		assert info.hasSeverity();
		assert info.hasSenderName();
		assert info.hasEvent();
		assert 1 == info.getAreaCount();
	...
	public Info getInfo() {
		assert 1 == getAlert().getInfoCount();
